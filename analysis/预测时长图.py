...
original_data = np.load('D:/Datasets/result/20250415/reversed/reverse_true_sst_SSTPredictor_mlp_SCS.npy')
predicted_data = np.load('D:/Datasets/result/20250415/reversed/reverse_pred_sst_SSTPredictor_mlp_SCS.npy')
# 计算有效预测时长
effective_duration = np.abs(original_data - predicted_data)
effective_duration = np.mean(effective_duration, axis=1)  # 取平均值,得到(2040, 64, 64)的数组
effective_duration = np.mean(effective_duration, axis=0)  # 再次取平均值,得到(2040,)的数组
