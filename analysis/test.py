...
end_date = dates.max()
for i in range(0, 5):
    true_values = true[2196:2562, lead[i]-1, 30, 30]  # 假设有5组数据，每个子图一个
    predicted_values = pred[2196:2562, lead[i]-1, 30, 30]
    ...
    # 计算每月RMSE并为每月的展示创建日期（每月的15日）
    monthly_rmse = data.groupby('Month')['Daily_RMSE'].mean().reset_index()
    monthly_rmse['DisplayDate'] = monthly_rmse['Month'].dt.to_timestamp() + pd.Timedelta(days=14)

    # axs[i].plot(data['Date'], data['Daily_RMSE'], label='Daily RMSE', alpha=1, linewidth = 2)
    axs[i].plot(data['Date'], predicted_values, label='DSTFN', alpha=1, linewidth = 3)
    axs[i].plot(data['Date'], true_values, label='OSTIA', alpha=1, linewidth = 3)

    # 在这里添加放大区域的代码
    # 定义放大区域的位置和大小
    axins = inset_axes(axs[i], width="80%", height="150%", loc="upper left",
                       bbox_to_anchor=(0.45, 0.26, 0.4, 0.4),
                       bbox_transform=axs[i].transAxes)
    # 在放大区域内绘制数据
    axins.plot(list(range(20)), predicted_values[213:233], label='DSTFN', alpha=1, linewidth=3)
    axins.plot(list(range(20)), true_values[213:233], label='OSTIA', alpha=1, linewidth=3)
    # 设置放大区域的x轴和y轴范围
    axins.set_xlim([0, 19])  # 调整为实际的索引范围或日期
    axins.set_ylim(min(true_values[213:233]) - 1, max(true_values[213:233]) + 1)
    ...
    # axs[i].scatter(monthly_rmse['DisplayDate'], monthly_rmse['Daily_RMSE'], color='red', label='Monthly RMSE', zorder=5)

    # # 在每个红点正上方添加数值标注，字体设置为'Times New Roman'
    # for j, row in monthly_rmse.iterrows():
    #     axs[i].text(row['DisplayDate'], row['Daily_RMSE'] + 0.05,  # 小幅度上移数值标注以避免与红点重叠
    #                 f"{row['Daily_RMSE']:.2f}", color='red', ha='center', va='bottom',
    #                 fontdict={'fontname': 'Times New Roman', 'weight': 'bold', 'fontsize': 18})


    # 在每个子图的左上角标记"L1"并设置字体样式
    axs[i].text(0.02, 0.9, 'L2', transform=axs[i].transAxes, fontdict={
    ...
    axs[i].set_ylabel('SST (℃)', fontdict={'family': 'Times New Roman', 'size': 20, 'weight': 'bold'})
...
# 自动调整子图布局
plt.tight_layout()
plt.savefig('D:/PythonProject/aos2023_sic_bs/大论文代码/图片/Daily-Monthly-RMSE_L2.png', format='png', dpi=300, bbox_inches='tight', pad_inches=0.1)
plt.show()
