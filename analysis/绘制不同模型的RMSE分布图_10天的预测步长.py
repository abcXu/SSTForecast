...
mae_list = np.load('../数据分析结果/20240701/days_rmse_later.npy')
# 获取最大值和最小值
...
# colors = ['#FF00FF', '#8000FF', '#0021FF', '#00A1FF', '#00E1FB', '#30FFC7', '#CAFF2C', '#FEED00', '#FFB200', '#FF3B00']
# colors = ['#FF0000','#FF3B00','#FF7700', '#FF9400', '#FFB200', '#FEED00', '#CAFF2C', '#97FF60',
# '#63FF94', '#49FFAD', '#30FFC7', '#00E1FB', '#00A1FF', '#0061FF', '#0021FF']
# colors = ['#DC0000','#FF0000','#FF3B00', '#FF7700', '#FF9400', '#FFB200', '#FEED00', '#CAFF2C',
#           '#97FF60',  '#49FFAD', '#30FFC7', '#00E1FB', '#00A1FF', '#0061FF', '#0021FF','#1D10CC']
...
for i in range(10):
    ...
    #
    # sns.heatmap(np.flipud(mae_list[i, 6, ...]), ax=axes[6][i], mask=np.flipud(mask), cbar=False, cmap=colors,
    #             xticklabels=False, yticklabels=False, square=True, vmax=vmax, vmin=vmin)
    # # 设置边框
    ...
    #
    # sns.heatmap(np.flipud(mae_list[i, 7, ...]), ax=axes[7][i], mask=np.flipud(mask), cbar=False, cmap=colors,
    #             xticklabels=False, yticklabels=False, square=True, vmax=vmax, vmin=vmin)
    # # 设置边框
...
# axes[7][0].text(-0.4, 0.5, 'DSTFN', fontsize=22, ha='center', va='center', transform=axes[7][0].transAxes, rotation=0, fontdict={'family':'Times New Roman', 'size': 6, 'weight': 'bold'})
# 调整子图之间的距离
...
cbar_ticks = np.linspace(0.4, 4.4, 9).tolist()
char_ticks=[]
...
# 注释掉
# # 添加共享的颜色条
# cbaxes = fig.add_axes([0.915, 0.12, 0.015, 0.75])
# # colorbar=fig.colorbar(axes[0][0].collections[0], ax=axes.ravel().tolist(), cax=cbar_ax, shrink=0.90, aspect=10)
# cbar = plt.colorbar(axes[0,0].imshow([[vmin, vmax]],
#                   cmap=plt.get_cmap('rainbow')),cax=cbaxes,
#                      orientation= 'vertical')
# cbar_ticks = np.linspace(0.5, 4, 8).tolist()
#
# colorbar = fig.colorbar(axes[0][0].collections[0], ax=axes.ravel().tolist(), cax=cbaxes, shrink=0.90, aspect=10,ticks=cbar_ticks)
# colorbar.ax.tick_params(labelsize=22)  # 设置图例刻度标签的字体大小为12
#
# colorbar.ax.yaxis.label.set_weight('bold')  # 设置标签加粗
# char_ticks=[]
# cbar.set_ticks(cbar_ticks)
...
plt.savefig('D:/Study/SwinLSTM-main/数据分析结果/20240701/Fig77.png', format='png', dpi=300, bbox_inches='tight', pad_inches=0)
plt.savefig('D:/Study/SwinLSTM-main/数据分析结果/20240701/Fig77.eps', format='eps',dpi=300, bbox_inches='tight', pad_inches=0)
