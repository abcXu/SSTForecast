...
# y_pred = np.load('D:/Datasets/result/20250415/reversed/reverse_pred_sst_SSTPredictor_mlp_SCS.npy')
# y_true = np.load('D:/Datasets/result/20250415/reversed/reverse_true_sst_SSTPredictor_mlp_SCS.npy')
...
# print(mask.shape)
# mae = mean_absolute_error(y_pred, y_true, mask)
#
...
def plot_error_maps(y_true, y_pred, mask, model_name='SSTPM'):
    days = y_true.shape[1]
    fig, axes = plt.subplots(1, days, figsize=(2.2 * days, 3), constrained_layout=True)
    order = ['{st}', '{nd}', '{rd}', '{th}','{th}', '{th}','{th}', '{th}','{th}', '{th}']
    for day in range(days):
        true_day = y_true[:, day, :, :]  # shape: (2040, 64, 64)
        pred_day = y_pred[:, day, :, :]

        # 均值误差图
        error_map = np.mean(pred_day - true_day, axis=0)  # shape: (64, 64)
        # mask陆地：设为NaN不显示
        error_map = np.where(mask == 1, error_map, np.nan)
        vmax = np.nanmax(error_map)
        vmin = np.nanmin(error_map)
        # 使用Normalize进行颜色归一化
        norm = mcolors.TwoSlopeNorm(vmin=vmin, vcenter=0, vmax=vmax)

        # 绘图
        ax = axes[day]
        im = ax.imshow(error_map, cmap='seismic', norm=norm,origin='lower')  # 选择seismic颜色映射
        ax.set_title(f'{day+1}$^{order[day]}$ day', fontsize=12)
        ax.axis('off')

    # 添加colorbar
    cbar = fig.colorbar(im, ax=axes, orientation='vertical', shrink=0.6, aspect=20, pad=0.02)
    cbar.set_label('SST Error (°C)', fontsize=12)

    plt.suptitle(f'{model_name} SST Prediction MAE Error Maps', fontsize=14)
    plt.savefig('pictures/mae_error_map.png',dpi=300, bbox_inches='tight', pad_inches=0)
    plt.show()
...
if __name__ == '__main__':
    ...
    y_true = np.load('D:/Datasets/result/20250415/reversed/reverse_true_sst_SSTPredictor_mlp_SCS.npy')
    print(y_pred.shape)
    print("max true sst_val:", np.max(y_true))
    print("min true sst_val:", np.min(y_true))
    print("max pred sst_val:", np.max(y_pred))
    print("min pred sst_val:", np.min(y_pred))
    mask = np.load('D:/GraduationThesis/codes/preData/maskLand=0.npy')
    plot_error_maps(y_true, y_pred, mask)