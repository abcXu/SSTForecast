...
rmse = np.load('../5个算法结果/20240525/days_names_mae.npy')
...
for i in range(10):
    ...
    # 在第一个子图的左侧添加标题PredRNN# ['ConvLSTM','SwinLSTM','ACFN','SE_ConvLSTM','SK_ConvLSTM']
    if(i==0):
        ax.text(-0.65, 0.5, 'SE_ConvLSTM', fontdict=fontdict, ha='center', va='center', transform=ax.transAxes, rotation=0)
        ax.set_xticks([])
    else:
        ax.set_yticks([])
    ax.set_ylim([0, 1570])  # 设置合适的范围
...
for i in range(10):
    ...
    # 在第一个子图的左侧添加标题PredRNN# ['ConvLSTM','SwinLSTM','ACFN','SE_ConvLSTM','SK_ConvLSTM']
    if(i==0):
        ax.text(-0.65, 0.5, 'SK_ConvLSTM', fontdict=fontdict, ha='center', va='center', transform=ax.transAxes, rotation=0)
        ax.set_xticks([])
    else:
        ax.set_yticks([])
    ax.set_ylim([0, 1570])  # 设置合适的范围
...
plt.savefig('../5个算法结果/20240525/mae误差高斯分布图111.eps', format='eps', dpi=300, bbox_inches='tight', pad_inches=0)
plt.savefig('../5个算法结果/20240525/mae误差高斯分布图111-eps-converted-to.pdf', format='pdf',  dpi=300, bbox_inches='tight', pad_inches=0)
plt.show()
